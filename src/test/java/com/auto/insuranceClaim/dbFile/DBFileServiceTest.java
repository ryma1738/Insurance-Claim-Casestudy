...
public class DBFileServiceTest {
    ...
    @Autowired private BDFileService dbFileService;
    ...
    @Test
    //must run this test first to test the userDownloadTest
    void mainTest() throws IOException {
        Path path = Paths.get("C:\\Users\\rymas\\Programing\\java-8\\Insurance-Claim-Casestudy\\src\\test\\java\\com\\auto\\insuranceClaim\\dbFile\\MockFile2.jpg");
        String name = "MockFile2.jpg";
        String originalFileName = "MockFile2.jpg";
        ...
        DBFile fileUploaded = dbFileService.storeFile(result, claim.getId());
        DBFile confirmUpload = claimRep.findById(claim.getId()).get().getDocuments().stream().collect(Collectors.toList()).get(0);
        assertEquals(fileUploaded.getFileName(), confirmUpload.getFileName());

        DBFile confirmDownload = dbFileService.getFile(confirmUpload.getId());
        assertEquals(confirmDownload.getFileName(), fileUploaded.getFileName());
    }

    @Test
    //Must run both tests for this one to pass
    void userDownloadTest() throws Exception {
        MultiValueMap<String, String> header = new LinkedMultiValueMap<>();
        header.add("Authorization", "Bearer " + userJwt);
        HttpHeaders headers = new HttpHeaders(header);
